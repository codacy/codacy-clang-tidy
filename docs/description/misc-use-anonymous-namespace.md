...
Standard. `static` was proposed for deprecation, but later un-deprecated
to keep C compatibility \[1\]. `static` is an overloaded term with
different meanings in different contexts, so it can create confusion.

The following uses of `static` will *not* be diagnosed:

- Functions or variables in header files, since anonymous namespaces in
  headers is considered an antipattern. Allowed header file extensions
  can be configured via the global option
  <span class="title-ref">HeaderFileExtensions</span>.
