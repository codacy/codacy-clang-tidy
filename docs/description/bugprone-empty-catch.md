...
exception but do nothing with it. This means that the exception is not
handled properly, and the program continues to run as if nothing
happened. This can lead to several issues, such as:

- *Hidden Bugs*: If an exception is caught and ignored, it can lead to
...
- *Security Issues*: Ignoring exceptions can lead to security issues,
...
- *Poor Code Quality*: Empty catch statements can indicate poor code
  quality and a lack of attention to detail. This can make the codebase
  difficult to maintain and update, leading to longer development cycles
  and increased costs.
- *Unreliable Code*: Code that ignores exceptions is often unreliable
  and can lead to unpredictable behavior. This can cause frustration for
...
warning will be raised. Default value:
<span class="title-ref">@TODO;@FIXME</span>.
