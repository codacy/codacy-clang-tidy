clang-tidy - altera-unroll-loops
...
- This check is unable to determine the number of iterations in a
  `while` or `do..while` loop; hence if such a loop is fully unrolled, a
  note is emitted advising the user to partially unroll instead.
...
can have. By default, it is set to <span class="title-ref">100</span>.
