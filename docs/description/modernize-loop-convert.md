...
Specifies the header file where `MakeReverseRangeFunction` is declared.
For the previous examples this option would be set to
`range/v3/view/reverse.hpp` and `llvm/ADT/STLExtras.h` respectively. If
this is an empty string and `MakeReverseRangeFunction` is set, the check
will proceed on the assumption that the function is already available in
the translation unit. This can be wrapped in angle brackets to signify
to add the include as a system include. Default value is an empty
string.
