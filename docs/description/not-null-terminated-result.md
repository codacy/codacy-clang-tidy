...
- If copy to the destination array can overflow \[1\] and
  `WantToUseSafeFunctions` is set to <span class="title-ref">true</span>
  and it is possible to obtain the capacity of the destination array
...
The value <span class="title-ref">true</span> specifies that the target
environment is considered to implement '\_s' suffixed memory and string
handler functions which are safer than older versions (e.g.
'memcpy_s()'). The default value is <span class="title-ref">true</span>.
