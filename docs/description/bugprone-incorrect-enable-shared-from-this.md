clang-tidy - bugprone-incorrect-enable-shared-from-this
...
Using <span class="title-ref">libstdc++</span> implementation,
`shared_from_this` will throw `std::bad_weak_ptr`. When
`using_not_public()` is called, this code will crash without exception
handling.
