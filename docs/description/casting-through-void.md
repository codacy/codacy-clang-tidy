...
This is intentional: `reinterpret_cast` is a dangerous operation that
can easily break the strict aliasing rules when dereferencing the casted
pointer, invoking Undefined Behavior. The warning is there to prompt
users to carefuly analyze whether the usage of `reinterpret_cast` is
