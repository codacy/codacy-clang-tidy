...
When `clang-tidy` is invoked with the
<span class="title-ref">--fix-notes</span> option, this check provides
fixes that automatically add the `inline` keyword to discovered
functions. Please note that the addition of the `inline` keyword to
variables is not currently supported by this check.
