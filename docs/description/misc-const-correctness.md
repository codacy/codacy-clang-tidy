...
The check does not run on <span class="title-ref">C</span> code.
...
Pointees can not be analyzed for constness yet. The following code shows
this limitation.
...
This limitation affects the capability to add `const` to methods which
is not possible, too.
...
does not analyze if the value-pointed-to is unchanged!
